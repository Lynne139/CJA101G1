...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ===== Modal - View =====
    ...
    //===== Modal Add + Edit 共用 =====
    //TinyMCE - 套用在textarea的編輯器，可透過gui編輯文檔後產出html
	...
	function bindImagePreview() {
  		...
  		const NO_IMAGE_FILENAME = "no_img.svg";
  		const NO_IMAGE_URL = "/images/admin/no_img.svg";
	
//	  	if (!input || !preview || !clearBtn || !clearFlag) return;
	
	  	// 初始原圖（若有圖就是 DB 圖，若無圖就是 no_img）
	  	...
	  	function updateClearButton() {
	    	const filename = getFilenameFromSrc(preview.src);
	  	...
	  	input.addEventListener("change", function () {
	    	...
	    	if (!file) {
	      		// 取消選擇 → 還原為原始圖片
			    preview.src = originalSrc || NO_IMAGE_URL;
		...
		clearBtn.addEventListener("click", () => {
	    	...
	    	input.value = "";
	    	preview.src = NO_IMAGE_URL;
	    	clearFlag.value = "true";
	    	updateClearButton();
  		});

  		// 最後呼叫一次確保按鈕狀態正確（畫面載入完成後）
